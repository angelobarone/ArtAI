...
def find_optimal_k(X, min_k, max_k):
    ...
    for k in tqdm(range(min_k, max_k + 1), desc="Elbow method calculating"):
        kmeans = KMeans(n_clusters=k, random_state=42)
        kmeans.fit(X)
        sse.append(kmeans.inertia_) #l'inerzia corrisponde all'sse

    #Trovare il punto di gomito con kneedle
    kneedle = KneeLocator(range(min_k, max_k + 1), sse, curve="convex", direction="decreasing")
    optimal_k = kneedle.knee

    #Grafico metodo del gomito
    plt.figure(figsize=(8, 5))
    plt.plot(range(min_k, max_k + 1), sse, marker='o', linestyle='--')
